...
class Label_Classification(object):
    def __init__(self, model_name='EnCAML', max_len=200, class_num=2, vocab_size=10000, embedding_size=100,
                 filter_num=300, kernel_size=4, padding='same', activation=None, dropout=10, batch_size=16,
                 learning_rates=0.0001, beta_1=0.9, beta_2=0.999, epochs=100, X_train=None, y_train=None, X_test=None, y_test=None):
    ...
    def train(self):
        ...
        model.fit(self.X_train, self.y_train, epochs=self.epochs, callbacks=[es], batch_size=self.batch_size, validation_split=0.2)
        
        test_loss, test_acc = model.evaluate(self.x_test, self.y_test)
        
        print("TEST Loss : {:.6f}".format(test_acc))
